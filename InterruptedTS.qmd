...
오늘 강의에서는 중단된 시계열 분석(Interrupted Time Series, ITS)이라는 강력한 통계 기법을 소개하고, 이를 산업보건 분야에 어떻게 적용할 수 있는지 살펴보겠습니다. 
ITS 분석은 특정 개입(intervention)이 시계열 데이터에 미치는 영향을 평가하는 데 사용됩니다. 
...
개입으로 인한 변화의 유형
개입으로 인해 데이터에서 나타날 수 있는 변화는 다음과 같은 두 가지 주요 형태를 가질 수 있습니다.
